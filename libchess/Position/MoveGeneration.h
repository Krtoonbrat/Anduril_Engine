...
inline void Position::generate_castling(MoveList& move_list, Color stm) const {
    ...
    Bitboard occupancy = occupancy_bb();
    if ((castling_possibilities[stm][0] & castling_rights().value()) &&
        !(castle_mask[stm][0] & occupancy) && !(attackers_to(castling_king_sqs[stm][0][0], !stm)) &&
        !(attackers_to(castling_intermediate_sqs[stm][0][0], !stm)) &&
        !(attackers_to(castling_intermediate_sqs[stm][0][1], !stm))) {
        move_list.add(
            Move{castling_king_sqs[stm][0][0], castling_king_sqs[stm][0][1], Move::Type::CASTLING});
    }
    if ((castling_possibilities[stm][1] & castling_rights().value()) &&
        !(castle_mask[stm][1] & occupancy) && !(attackers_to(castling_king_sqs[stm][0][0], !stm)) &&
        !(attackers_to(castling_intermediate_sqs[stm][1][0], !stm)) &&
        !(attackers_to(castling_intermediate_sqs[stm][1][1], !stm))) {
        move_list.add(
            Move{castling_king_sqs[stm][1][0], castling_king_sqs[stm][1][1], Move::Type::CASTLING});
    }
}

inline void Position::generate_checker_block_moves(MoveList& move_list, Color stm) const {
    Bitboard checkers = checkers_to(stm);
    ...
    Bitboard pawns = piece_type_bb(constants::PAWN, stm);
    Bitboard shifted_intercepts = lookups::pawn_shift(checker_intercept_bb, !stm);
    Bitboard single_push_pawn_blocks = shifted_intercepts & pawns;
    Bitboard double_push_pawn_blocks =
            lookups::pawn_shift(shifted_intercepts & ~occupancy_bb(), !stm) & pawns &
            lookups::relative_rank_mask(constants::RANK_2, stm);
    while (double_push_pawn_blocks) {
        ...
        move_list.add(Move{pawn_sq, lookups::pawn_shift(pawn_sq, stm, 2), Move::Type::DOUBLE_PUSH});
    }
    while (single_push_pawn_blocks) {
        ...
        Square target_sq = lookups::pawn_shift(pawn_sq, stm);
        if ((stm == constants::WHITE && pawn_sq.rank() == constants::RANK_7) ||
            (stm == constants::BLACK && pawn_sq.rank() == constants::RANK_2)) {
    ...
    while (checker_intercept_bb) {
        ...
        Bitboard blockers = attackers_to(sq, stm) & excluded_pieces_mask;
        while (blockers) {
...
inline void Position::generate_checker_capture_moves(MoveList& move_list, Color stm) const {
    Bitboard checkers = checkers_to(stm);
    if (checkers.popcount() > 1) {
        return;
    }
    ...
    if (ep_square && (lookups::pawn_shift(Bitboard{*ep_square}, !stm) & checkers)) {
        Bitboard ep_candidates = pawns & lookups::pawn_attacks(*ep_square, !stm);
        while (ep_candidates) {
            Square sq = ep_candidates.forward_bitscan();
            ep_candidates.forward_popbit();
    ...
    Bitboard attackers = attackers_to(checker_sq, stm) & ~Bitboard{king_square(stm)};
    Bitboard rank7_pawns = pawns & lookups::relative_rank_mask(constants::RANK_7, stm);
...
inline MoveList Position::check_evasion_move_list(Color stm) const {
    MoveList move_list;
    Square king_sq = king_square(stm);
    Bitboard checkers = checkers_to(stm);
    Bitboard non_king_occupancy = occupancy_bb() ^ Bitboard { king_sq };

    Bitboard evasions = lookups::king_attacks(king_sq) & ~color_bb(stm);
    Bitboard opp_occupancy = color_bb(!stm);
    while (evasions) {
        Square sq = evasions.forward_bitscan();
        evasions.forward_popbit();
        if (!(attackers_to(sq, non_king_occupancy, !stm))) {
    ...
    generate_checker_capture_moves(move_list, stm);
    generate_checker_block_moves(move_list, stm);
    return move_list;
}
