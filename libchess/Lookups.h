...
struct MagicAttacksLookup {
    std::vector<PlainMagicLookup> magic_lookup;
    std::vector<std::vector<Bitboard>> attacks;
};

namespace init {

inline MagicAttacksLookup<SlidingPieceType::ROOK> rook_magic_attacks_lookup() {
    std::vector<PlainMagicLookup> magic_lookup(64);
    std::vector<std::vector<Bitboard>> attacks(64);
    for (auto & attack : attacks) {
        attack = std::vector<Bitboard>(4096);
    }
    std::vector<Bitboard> occupancy_combinations(4096);
    std::vector<Bitboard> legal_sliding_attacks(4096);
    ...
    MagicAttacksLookup<SlidingPieceType::ROOK> lookup;
    lookup.magic_lookup = magic_lookup;
    lookup.attacks = attacks;
    return lookup;
}
...
inline MagicAttacksLookup<SlidingPieceType::BISHOP> bishop_magic_attacks_lookup() {
    ...
    std::vector<std::vector<Bitboard>> attacks(64);
    for (auto & attack : attacks) {
        attack = std::vector<Bitboard>(512);
    }
    std::vector<Bitboard> occupancy_combinations(512);
    std::vector<Bitboard> legal_sliding_attacks(512);
    ...
    MagicAttacksLookup<SlidingPieceType::BISHOP> lookup;
    lookup.magic_lookup = magic_lookup;
    lookup.attacks = attacks;
