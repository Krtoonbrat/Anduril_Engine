...
std::string ChessPosition::ToDebugStr( const char *label )
{
    ...
    for( int row=0; row<8; row++ )
    {
        ...
        // start the row with a vertical line
        s += " |";

        // iterate through each item in the current row
        // if there isn't a piece, 7 spaces are printed
        ...
        for( int col=0; col<8; col++ )
        {
            ...
            // add the pipe at the end of each square
            s += "|";
        }
    ...
    // add the letters at the bottom of the board
    s += "      a       b       c       d       e       f       g       h    ";
...
void ChessRules::PlayMove( Move imove )
{
    ...
    // moved to PushMove so that the move count updates
    // as the engine tests moves
    //if( !white )
    //   full_move_count++;
...
void ChessRules::PushMove( Move& m )
{
    ...
    // added by Krtoonbrat
    // just toggles the turn if a null move is passed in
    if (!m.Valid()) {
        // Toggle who-to-move
        Toggle();
...
void ChessRules::PopMove( Move& m )
{
    ...
    // added by Krtoonbrat
    // decrement the full move counter
    if (white) {
        full_move_count--;
    }

    // remove from move history
    history[--history_idx].Invalid();

    // Toggle who-to-move
    Toggle();

    // added by Krtoonbrat
    // skip the rest if the move was null
    if (!m.Valid()) {
        return;
    }
